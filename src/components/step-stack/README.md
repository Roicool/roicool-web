...
duran tek bir görsel çerçevesi. Her adımın kendi görseli var ve adım
satırının üst çizgisi onu taşır: çizgi çerçevenin altından girip yukarı
çıkarken görsel o çizgiye yapışık olarak çerçeveye girer, öncekinin üstüne
biner, önceki altında kararıp küçülür; çizgi çerçevenin üstüne değince
görsel tam yerindedir. Geri kaydırınca aynı yoldan çıkar. Kaynak:
webnomads.com "Driving growth by design".

**Hareket scroll'a bağlı, zamana değil.** Her karede her görselin yeri
adımının çizgisinden hesaplanır: çerçeve hızlı kaydırmanın gerisinde
kalamaz, geçişler kuyruklanmaz, yön değişince aynı yol geriye oynar.
Varsayılan birebir; istenirse kısa bir scrub (`data-rc-scrub`) görselleri
çizgiden biraz geriden getirir.
...
| `data-rc-line`     | %, `50`     | Adımın güncel sayıldığı yer: çerçevenin üstünden, çerçeve yüksekliğinin yüzdesi; çizgisi burayı geçen adım günceldir (metin vurgusu, video). Görseli çizgi taşır, bundan bağımsız. Dar ekranda viewport yüksekliğinin yüzdesi |
| `data-rc-scrub`    | saniye, `0` | Görsellerin çizgiyi izleme gecikmesi; `0` birebir                                                                                                                                                                             |
...
- **Dar ekran:** stage body'nin ilk çocuğu olarak üstte, normal akışta;
  ...
  yalnız adım vurgusu (aşağıdaki "Metin") çalışır: çizgisi viewport'un
  `line`%'ini geçen son adım günceldir. Pencere genişleyip daralınca kod
  çerçeveyi o anki adıma anında getirir.
- **Çizgi görseli taşır:** k. adımın üst kenarı (satırın çizgisi) çerçevenin
  altındayken görseli çerçevenin altında bekler; çizgi çerçevenin içinden
  yukarı çıkarken görselin üst kenarı çizginin üstünde gider (`translate`
  birebir, içindeki görsel `parallax` kadar geriden); çizgi çerçevenin
  üstüne değince görsel tam yerindedir. Aynı oranda alttaki görsel kararır
  (`::after` katmanı, `1 − dim`) ve `%96`'ya küçülür.
- **Section'a girerken hiçbir görsel değişmez:** çerçeve yapışana kadar
  adımlarla birlikte kayar, çizgiler ona göre yer değiştirmez. Çerçeve
  yapışınca çizgiler altından geçmeye başlar ve her biri kendi görselini
  getirir. (Bir satır çerçeveden kısaysa sonraki çizgi çerçevenin içine
  düşer ve görseli o kadar erken girer; satır yüksekliğini çerçeveden az
  tutma.)
- **Güncel adım:** çizgisi çerçevenin üstünden `line`%'i geçmiş son adım.
  Metin vurgusu ve video o an değişir; görsel o anda çerçevenin o kadarını
  kaplamıştır.
- **Scrub:** `data-rc-scrub` verilirse görseller çizgiyi o kadar saniyelik
  üstel gecikmeyle izler; varsayılan birebir. Yığın ekran dışındayken
